...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack }) => {
  ...
  const [filterProfilePic, setFilterProfilePic] = useState<'all' | 'with' | 'without'>('all');
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8">
          {activeTab === 'posts' ? (
            ...
            <div className="space-y-6">
              {/* Search and Filters */}
              <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-rose-100 dark:border-gray-700 space-y-4">
                <div className="relative">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-rose-300" />
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="ইউজার আইডি, নাম বা মোবাইল নম্বর দিয়ে খুঁজুন..."
                    className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-rose-50 dark:border-gray-700 focus:border-rose-400 outline-none transition-all bg-rose-50/30 dark:bg-gray-900/30 dark:text-white"
                  />
                </div>
                
                <div className="flex flex-wrap gap-4">
                  <div className="flex-1 min-w-[150px]">
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2 ml-1">সর্টিং</label>
                    <select 
                      value={sortOrder}
                      onChange={(e) => setSortOrder(e.target.value as any)}
                      className="w-full px-4 py-2.5 rounded-xl border border-rose-100 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm outline-none focus:border-rose-400 dark:text-white"
                    >
                      <option value="newest">নতুন আগে</option>
                      <option value="oldest">পুরানো আগে</option>
                    </select>
                  </div>
                  
                  <div className="flex-1 min-w-[150px]">
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2 ml-1">অনলাইন স্ট্যাটাস</label>
                    <select 
                      value={filterOnline}
                  ...
                  <div className="flex-1 min-w-[150px]">
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2 ml-1">প্রোফাইল পিকচার</label>
                    <select 
                      value={filterProfilePic}
                      onChange={(e) => setFilterProfilePic(e.target.value as any)}
                      className="w-full px-4 py-2.5 rounded-xl border border-rose-100 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm outline-none focus:border-rose-400 dark:text-white"
                    >
                      <option value="all">সবাই</option>
                      <option value="with">আছে</option>
                      <option value="without">নেই</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-4">
                {userAccounts
                  .filter(acc => {
                    const profile = allProfiles[acc.userId];
                    const matchesSearch = 
                    ...
                    const matchesOnline = 
                      filterOnline === 'all' || 
                      (filterOnline === 'online' && profile?.isOnline) ||
                      (filterOnline === 'offline' && !profile?.isOnline);
                    
                    const matchesPic = 
                      filterProfilePic === 'all' ||
                      (filterProfilePic === 'with' && profile?.profileImageUrl) ||
                      (filterProfilePic === 'without' && !profile?.profileImageUrl);
                    
                    return matchesSearch && matchesOnline && matchesPic;
                  })
                  .sort((a, b) => {
                  ...
                  .map((acc) => (
                    <div key={acc.id} className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-xl border border-rose-100 dark:border-gray-700 hover:shadow-2xl transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <div className="w-14 h-14 rounded-2xl bg-rose-50 dark:bg-gray-700 flex items-center justify-center text-rose-500 font-bold text-2xl border border-rose-100 dark:border-gray-600 overflow-hidden">
                            {allProfiles[acc.userId]?.profileImageUrl ? (
                              <img src={allProfiles[acc.userId].profileImageUrl} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                            ) : (
                              (acc.userId || 'U').charAt(0).toUpperCase()
                            )}
                          </div>
                          {allProfiles[acc.userId]?.isOnline && (
