...
import { Send, User, ArrowLeft, MessageSquare, Image as ImageIcon, Trash2, Shield, Users, Link as LinkIcon, X, Check, UserPlus, Plus, Edit, Camera, Video, Loader2 } from 'lucide-react';
...
const GroupChat: React.FC<GroupChatProps> = ({ userId, userName, group: initialGroup, friends, onBack }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if ((input.trim() || selectedImage || selectedVideo) && socket && !isSending) {
      setIsSending(true);
      setUploadProgress(0);
      ...
      try {
        ...
        if (selectedFile) {
          const url = await uploadFile(selectedFile, selectedFile.name, (progress) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-rose-50 max-w-2xl mx-auto shadow-2xl relative">
                  ...
                  `}>
                    ...
                    {msg.videoUrl && (
                      <video 
                        src={msg.videoUrl} 
                        controls 
                        className="w-full max-h-60 rounded-xl mb-2 border border-black/5" 
                      />
                    )}
      ...
      {/* Image/Video Preview & Progress */}
      <AnimatePresence>
        {(selectedImage || selectedVideo || uploadProgress !== null) && (
          <motion.div 
            initial={{ y: 100 }}
            animate={{ y: 0 }}
            exit={{ y: 100 }}
            className="absolute bottom-20 left-4 right-4 bg-white p-2 rounded-2xl shadow-2xl border border-rose-100 z-20 overflow-hidden"
          >
            {uploadProgress !== null && (
              <div className="absolute top-0 left-0 h-1 bg-rose-500 transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
            )}
            {(selectedImage || selectedVideo) && (
              <div className="relative">
                {selectedImage ? (
                  ...
                  <video src={selectedVideo!} className="w-full h-32 object-cover rounded-xl" />
                )}
                <button 
                  onClick={() => {
                    setSelectedImage(null);
      ...
      <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-rose-100 z-10">
        <div className="flex gap-2 items-center">
          ...
          <button
            type="submit"
            disabled={(!input.trim() && !selectedImage && !selectedVideo) || isSending}
